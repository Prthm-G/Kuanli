...
async function runStep(
  ...
  switch (step.step_type) {
    ...
    case 'send_webhook': {
      ...
      // SSRF guard: the URL is account-controlled and the server makes
      // the request, so refuse any destination resolving to a private /
      // loopback / link-local / reserved address. Without this, an
      // automation author reaches every service on the compose network.
      ...
      const res = await fetch(cfg.url, {
        ...
        // so a hung internal host can't tie up the runner.
        redirect: 'manual',

...
const META_API_VERSION = process.env.AURETRIS_META_GRAPH_VERSION ?? 'v25.0';
/**
 * Exported so the Conversions API client (src/lib/meta/conversions.ts) posts to
 * the same Graph version this file already sends messages on. A second copy of
 * the version string is the exact shape of defect this project keeps finding -
 * a rule held in two places and kept in sync by eyeball.
 */

...
export async function ensureImageHeaderHandle(
  ...
  // member can submit a template) and the fetch below happens
  // server-side, so refuse any destination resolving to a private /
  // loopback / link-local / reserved address. Reusing the unreachable-host
  // message keeps the failure from being an internal-host oracle.
  ...
  let res: Response;
  try {
    res = await fetch(payload.header_media_url, { redirect: 'manual' });
  } catch {
    throw new Error(
      'Could not fetch the header image URL. Make sure it is publicly reachable.'
    );
